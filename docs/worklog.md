...
- `api/movevalue_api.py`: `/api/geocode`, `/api/commute-route` 추가. `/api/health`는 ODsay/TMAP/Kakao 키 설정 여부를 true/false로만 반환
- 상세 근거 패널: 선택 생활권 좌표와 회사 주소/좌표를 넣어 총 소요시간, 환승, 도보, 요금, 단계별 경로를 확인하는 UI 추가
...
1. ODsay/TMAP API 키 운영, 호출 캐시, 실패 로그, 혼잡도, 첫차·막차
