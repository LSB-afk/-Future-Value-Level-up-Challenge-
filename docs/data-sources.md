...
현재 한계는 통근시간이 API 키 미설정 환경에서 테이블 폴백으로 생성된다는 점과, 안전·환경 점수가 MVP 프록시라는 점이다. 이 한계는 웹 화면(데이터 근거 섹션, 점수 보조 문구)에도 동일하게 고지한다. 배포 단계에서는 경로 API 키 운영, 목적지 자유 입력, 안전·대기·녹지 공공데이터 어댑터를 추가한다.
