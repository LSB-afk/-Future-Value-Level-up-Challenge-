...
- 구현 파일: `api/property_model.py`, `api/property_adapters.py`, `app/app.js`
- 원천 실데이터: 서울시 공동주택 아파트 정보의 단지 기본 정보와 서울시 2025 전월세 실거래 생활권 집계
- 연계 예정 실데이터: 국토교통부 아파트 매매 실거래가 API, 아파트 전월세 실거래가 API, 공동주택 공시가격, 건축물대장, VWorld 지오코더/용도지역
- 지도 표시: `/api/apartments`의 `features[].pricePreview`가 단지별 추정 매매가, 전세가율, 위험 신호 등급을 포함한다.
- 상세 응답: `/api/property-detail?id=A15275101`이 기본 정보, 가격 정보, 12개월 거래 추이, 전세 위험 신호, 생활권 정보, AI 요약, 데이터 연계 상태를 반환한다.
- AI Agent: `/api/property-agent?id=A15275101&question=...`이 전세 안전성, 추천 이유, 더 안전한 비교 후보 질문에 근거 목록과 함께 답한다.
- 데이터 고지: 매매가·공시가격·거래 추이는 API 키 없이도 화면 검증이 가능하도록 생활권 전월세 집계 기반 추정값으로 생성한다. 화면의 `데이터 연계 상태`에서 실데이터/추정/연계 예정 여부를 구분한다.
- 위험 신호: 전세가율, 공시가격 대비 보증금, 주변 전세 중앙값 대비, 변동성, 건축물 노후도, 등기부 권리관계 미입력 여부를 점수화하되 법적 판정으로 표현하지 않는다.
...
7. 사용자가 단지를 클릭하면 공개 단지정보와 생활권 전월세 집계를 결합해 상세 대시보드와 전세 위험 신호 점검 결과를 보여준다.
...
현재 한계는 기본 추천 데이터의 통근시간이 API 키 미설정 환경에서 테이블 폴백으로 생성된다는 점, 사용자 통근 루트가 키 없을 때 거리 기반 폴백이라는 점, 안전·환경이 자동 원천 API 갱신이 아닌 스냅샷 점수라는 점이다. 아파트 단지 레이어도 키 없는 커밋 상태에서는 sample 제한 때문에 5건 미리보기만 포함한다. 이 한계는 웹 화면(데이터 근거 섹션, 점수 보조 문구, 통근 루트 결과 notice, 지도 레이어 상태 배지)에도 동일하게 고지한다. 배포 단계에서는 경로 API 키 운영, 호출 캐시, 혼잡도·첫차·막차, 안전·대기·녹지 전체 원천 API 자동 갱신, 서울 전체 공동주택 단지 갱신 배치를 추가한다.
