...
6. 사용자의 예산, 통근 목적지, 가구 유형, 접근성 우선순위에 맞춰 설명 가능한 점수를 제공한다.
...
현재 한계는 기본 추천 데이터의 통근시간이 API 키 미설정 환경에서 테이블 폴백으로 생성된다는 점, 사용자 통근 루트가 키 없을 때 거리 기반 폴백이라는 점, 안전·환경이 자동 원천 API 갱신이 아닌 스냅샷 점수라는 점이다. 이 한계는 웹 화면(데이터 근거 섹션, 점수 보조 문구, 통근 루트 결과 notice)에도 동일하게 고지한다. 배포 단계에서는 경로 API 키 운영, 호출 캐시, 혼잡도·첫차·막차, 안전·대기·녹지 전체 원천 API 자동 갱신을 추가한다.
