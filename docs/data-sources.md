...
웹 서비스의 "데이터 근거" 섹션은 `GET /api/areas` 응답의 `evidence` 필드를 그대로 표로 렌더링한다. 생활권별 매칭 거래 건수(총 114,319건), 법정동 범위, 월세·보증금·전세 중앙값이 화면에서 확인되며, 상세 패널에서도 선택한 생활권의 실데이터 근거를 항목별로 보여준다.
...
현재 한계는 통근시간·생활 SOC·안전·환경 일부 지표가 MVP 프록시라는 점이다. 이 한계는 웹 화면(데이터 근거 섹션, 점수 보조 문구)에도 동일하게 고지한다. 배포 단계에서는 대중교통 경로 API, 생활시설 좌표, 안전·환경 공공데이터를 각각 독립 어댑터로 추가한다.
