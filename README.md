...
웹 화면은 단일 페이지 서비스로, 상단 메뉴(추천 · 지도 · 데이터 근거 · API · 사업모델 · 제출자료)를 탭처럼 전환합니다. 각 메뉴를 누르면 해당 화면 하나만 표시됩니다.
...
- `scripts/movevalue_adapters.py`: 대중교통 경로 API 폴백 어댑터와 생활 SOC 좌표 집계 어댑터
...
주거비(월세·보증금·전세 중앙값)는 서울시 2025 전월세 실데이터에서 집계한 값입니다. 생활 SOC는 병의원·학교·공원 좌표 스냅샷 반경 집계 기반입니다. 통근시간은 경로 API 어댑터가 있으나 키 미설정 환경에서는 테이블 폴백으로 생성됩니다. 안전·환경 점수는 MVP 검증용 프록시이며, 선정 후 구체화 단계에서 실제 공공데이터로 고도화합니다(`docs/product-roadmap.md` 참고).
