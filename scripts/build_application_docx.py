...
def add_budget_table(doc):
    rows = [
        ("데이터 수집·정제", "5,000천원", "API 연계, 데이터 정규화, 품질 점검"),
        ("프로토타입 개발", "8,000천원", "웹 UI, 점수 엔진, API 어댑터"),
        ("지도·분석 고도화", "3,000천원", "지도 시각화, 생활권 분석"),
...
def build():
    ...
    add_kv_table(
        ...
        [
            ...
            ("산출물", "웹 기반 생활권 추천 프로토타입, 점수 산정 엔진, 데이터 연계 설계서, 서비스 기획서"),
            ("소요 예산", "20,000천원"),
    ...
    sections = [
        ...
        (
            "실증서비스 아이디어 모델 개요",
            "MoveValue는 사용자의 월 주거 예산, 출퇴근 목적지, 가구 유형, 생활 인프라 우선순위를 입력받아 후보 생활권을 추천하는 서비스입니다. 단순 월세나 매매가가 아니라 주거비, 이동시간, 환승 편의, 생활 SOC 접근성, 안전·환경 지표를 함께 계산합니다. 핵심 기능은 생활권 랭킹, 항목별 점수 설명, 후보지 비교, 정책·플랫폼용 API 제공입니다.",
        ),
        (
            "배경 및 필요성",
        ...
        (
            "차별성",
            "기존 부동산 서비스는 매물 가격과 위치 검색 중심입니다. MoveValue는 거주 후 매월 발생하는 이동 부담과 생활 접근성을 비용화·점수화합니다. 추천 결과를 블랙박스로 제시하지 않고 주거비·통근·교통 접근성·생활 SOC·환경 항목별 근거를 함께 보여줍니다. 공공데이터만으로 최소 기능을 구현할 수 있고, 민간 이동 데이터나 플랫폼 데이터가 붙으면 정밀도를 높일 수 있는 단계형 구조입니다.",
        ),
        (
            "현황 및 구체화 방안",
            "현재 작업공간에는 서울시 2025 전월세 실데이터를 집계한 API 기반 웹 프로토타입이 구현되어 있습니다. 선정 후에는 국토교통부 실거래가 API, VWorld 지오코더, 대중교통 경로 API, 전국도시철도 역사정보, 국가교통 데이터 오픈마켓 데이터를 추가 어댑터로 연결합니다. 최종 산출물은 웹 프로토타입 1식, 데이터 연계 모듈 1식, 생활권 점수 산정 API 1식, 지자체·플랫폼용 리포트 예시 1식으로 구성합니다.",
        ),
        (
            "서비스 구현을 위한 협력 방안",
