...
def build():
    ...
    sections = [
        ...
        (
            "현황 및 구체화 방안",
            "현재 작업공간에는 샘플 데이터 기반 웹 프로토타입이 구현되어 있습니다. 선정 후에는 국토교통부 실거래가 API, VWorld 지오코더, 전국도시철도 역사정보, 국가교통 데이터 오픈마켓 데이터를 실제 어댑터로 연결합니다. 최종 산출물은 웹 프로토타입 1식, 데이터 연계 모듈 1식, 생활권 점수 산정 로직 1식, 지자체·플랫폼용 리포트 예시 1식으로 구성합니다.",
        ),
        (
            "서비스 구현을 위한 협력 방안",
